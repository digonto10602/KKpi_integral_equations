...
int main(int argc, char* argv[])
{
    ...
    if(argc==1)
    {
        std::cout<<"input needed: <debug_y/n> <m1> <m2> "
                 <<"<a0_1> <a0_2> <eps_for_ope> <eps_for_cutoff> "
    ...
    else 
    {
        ...
        double eps_for_ope = std::strtod(argv[6], &endptr); 
        double eps_for_cutoff = std::strtod(argv[7], &endptr); 
        double En_initial = std::strtod(argv[8], &endptr); 
        double del_ext_En = std::strtod(argv[9], &endptr); 
        double En_points = std::strtod(argv[10], &endptr); 
        double En_imag = std::strtod(argv[11], &endptr); 
        double mom_points = std::strtod(argv[12], &endptr); 
        double preal_val = std::strtod(argv[13], &endptr); 
        double pimag_val = std::strtod(argv[14], &endptr); 
        std::string file_initial = argv[15]; 
        std::string dest_to_py_script = argv[16]; 
