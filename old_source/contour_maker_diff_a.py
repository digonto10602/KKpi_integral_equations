...
def shifttest2_fitting_code_N_linear(filename, reguessA, reguessB, imguessA, imguessB):
    #fout<<setprecision(16)<<s<<'\t'<<eps<<'\t'<<size<<'\t'<<real(result)<<'\t'<<imag(result)<<endl;
    reguess = [reguessA,reguessB]
    imguess = [imguessA,imguessB]
    (res, ims, N,size1, reRes, imRes) = np.genfromtxt(filename,unpack=True)
    reParameters, reCovariance = curve_fit(N_linear_fitting_function,N,reRes, p0=reguess) 
    imParameters, imCovariance = curve_fit(N_linear_fitting_function,N,imRes, p0=imguess) 
    reFitA = reParameters[0]
    reFitB = reParameters[1]
...
def plotFit3( filename1, filename2, filename3 ):
    ...
    (res1, ims1, N1, size1, reRes1, imRes1) = np.genfromtxt(filename1, unpack=True)
    (res2, ims2, N2, size2, reRes2, imRes2) = np.genfromtxt(filename2, unpack=True)
    (res3, ims3, N3, size3, reRes3, imRes3) = np.genfromtxt(filename3, unpack=True)
    ...
    (reFitA1, reFitB1, SE_reFitA1, SE_reFitB1, imFitA1, imFitB1, SE_imFitA1, SE_imFitB1) = shifttest2_fitting_code_N_linear(filename1,1.0,1.0,1.0,1.0)
    (reFitA2, reFitB2, SE_reFitA2, SE_reFitB2, imFitA2, imFitB2, SE_imFitA2, SE_imFitB2) = shifttest2_fitting_code_N_linear(filename2,1.0,1.0,1.0,1.0)
    (reFitA3, reFitB3, SE_reFitA3, SE_reFitB3, imFitA3, imFitB3, SE_imFitA3, SE_imFitB3) = shifttest2_fitting_code_N_linear(filename3,1.0,1.0,1.0,1.0)


    print(reFitA1,reFitA2,reFitA3)
    print(imFitA1,imFitA2,imFitA3)
    ax[0].scatter(N1,reRes1,s=100, marker="o", color='red', alpha=0.55, label="contour 1" )
    ax[0].plot(N1,N_linear_fitting_function(N1,reFitA1,reFitB1),linestyle='solid', linewidth=2, color='red' )
    ax[0].axhline(y=reFitA1,linestyle='dashed', linewidth=2, color='red', label="contour 1, $N \\rightarrow \\infty$" )
    ax[0].scatter(N2,reRes2,s=100, marker="o", color='green', alpha=0.55, label="contour 4" )
    ax[0].plot(N2,N_linear_fitting_function(N2,reFitA2,reFitB2),linestyle='solid', linewidth=2, color='green' )
    ax[0].axhline(y=reFitA2,linestyle='dashed', linewidth=2, color='green', label="contour 4, $N \\rightarrow \\infty$" )
    ax[0].scatter(N3,reRes3,s=100, marker="s", color='blue', alpha=0.55, label="contour 5" )
    ax[0].plot(N3,N_linear_fitting_function(N3,reFitA3,reFitB3),linestyle='solid', linewidth=2, color='blue' )
    ax[0].axhline(y=reFitA3,linestyle='dashed', linewidth=2, color='blue', label="contour 5, $N \\rightarrow \\infty$" )

    
    ax[1].scatter(N1,imRes1,s=100, marker="o", color='red', alpha=0.55, label="contour 1" )
    ax[1].plot(N1,N_linear_fitting_function(N1,imFitA1,imFitB1), linestyle='solid', linewidth=2, color='red' )
    ax[1].axhline(y=imFitA1,linestyle='dashed', linewidth=2, color='red', label="contour 1, $N \\rightarrow \\infty$" )
    ax[1].scatter(N2,imRes2,s=100, marker="o", color='green', alpha=0.55, label="contour 4" )
    ax[1].plot(N2,N_linear_fitting_function(N2,imFitA2,imFitB2), linestyle='solid', linewidth=2, color='green' )
    ax[1].axhline(y=imFitA2,linestyle='dashed', linewidth=2, color='green', label="contour 4, $N \\rightarrow \\infty$" )
    ax[1].scatter(N3,imRes3,s=100, marker="s", color='blue', alpha=0.55, label="contour 5" )
    ax[1].plot(N3,N_linear_fitting_function(N3,imFitA3,imFitB3), linestyle='solid', linewidth=2, color='blue' )
    ax[1].axhline(y=imFitA3,linestyle='dashed', linewidth=2, color='blue', label="contour 5, $N \\rightarrow \\infty$" )

    output_file = "shifttest2_Mphib_contour_comparison_linear_fit_3files.pdf"
...
def print_contour_files(opefile,qvecfile,pcutfile,thresholdfile,opecuttracerfile,scount,N):
    ...
    ax.contour(Xi, Yi, zi,levels=np.linspace(zi.min(),zi.max(),num=250), linewidths=0.5, colors='k',linestyles='solid')
    h0 = ax.contourf(Xi, Yi, zi,levels=np.linspace(zi.min(),zi.max(),num=250))
   ...
   #ax[1].plot(req,imq, marker="o", markersize=3, markeredgecolor="black", markerfacecolor="darkviolet")
    ...
    outputfile_str = "glockletest_" + pcutfile + ".jpg"
...
filename1 = "shifttest2_kernel_1_smoothcutoff.dat"
filename2 = "shifttest2_kernel_4_smoothcutoff.dat"
filename3 = "shifttest2_kernel_5_smoothcutoff.dat"
