...
void LUSolver_complex_cuda( Eigen::MatrixXcd &A1,
                            Eigen::VectorXcd &B1,
                            Eigen::VectorXcd &X1,
                            int matsize,
                            std::complex<double> &determinant_of_A )
{
    cusolverDnHandle_t cusolverH = NULL;
    cudaStream_t stream = NULL;

    using data_type = cuDoubleComplex;//complex<double>();

    const int64_t m = matsize;
    const int64_t lda = m;
    const int64_t ldb = m;
    /*       | 1 2 3  |
     *   A = | 4 5 6  |
    ...
    for(int i=0;i<lda*m;++i) LU.push_back(make_cuDoubleComplex(0.0,0.0));
    ...
    {
        cuDoubleComplex tempB = make_cuDoubleComplex(B1(i).real(),B1(i).imag());
        B.push_back( tempB );
        for(int j=0;j<matsize;++j)
        {
            cuDoubleComplex tempA = make_cuDoubleComplex(A1(j,i).real(),A1(j,i).imag());
            A.push_back( tempA );
        }
    }
    ...
    const int pivot_on = 0; // 1 means pivot is on, 0 means pivot is off
    const int algo = 0;
    ...
    /*for(int i=0;i<matsize;++i)
    {
        X1[i] = cuCreal(X[i]) + ii*cuCimag(X[i]);
    }*/

    std::complex<double> temp_det = {1.0,0.0};
    for(int i=0;i<matsize;++i)
    {
        for(int j=0;j<matsize;++j)
        {
            if(i==j)
            {
                temp_det = temp_det*(cuCreal(LU[matsize*j + i]) + ii*cuCimag(LU[matsize*j + i]));
            }
        }
    }

    //std::cout<<"im here"<<endl;
    determinant_of_A = temp_det;
